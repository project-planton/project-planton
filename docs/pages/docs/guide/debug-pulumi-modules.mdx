...
2. **Set the `STACK_INPUT_FILE_PATH` environment variable** to point to your stack input file.
...
<iframe width="640" height="360" src="https://www.loom.com/embed/80bdb123c9d34e6e8ed453fd466effcb?sid=f752ad17-b9f6-4586-a9ee-6442fdb3e4db" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>
...
### 3. Set the `STACK_INPUT_FILE_PATH` Environment Variable

All ProjectPlanton maintained Pulumi modules rely on an environment variable called `STACK_INPUT_FILE_PATH`, which contains the path to the stack input file. This file defines the configuration for the deployment component, following the protobuf definitions specified by ProjectPlanton.

Before running `pulumi up`, you need to set this environment variable.

#### Example `stack-input.yaml` File

Create a `stack-input.yaml` file with your deployment configuration. Here's an example:
...
- **Explanation:**
    - The `stack-input.yaml` contains a `target` field that includes the API resource definition.
...
Set the `STACK_INPUT_FILE_PATH` environment variable to point to your `stack-input.yaml` file.

```bash
export STACK_INPUT_FILE_PATH=/path/to/your/stack-input.yaml
```

- **Note:**
    - Ensure that the path is correct and accessible.
    - run it in the terminal before running `pulumi up`.
...
Remember to set the `STACK_INPUT_FILE_PATH` environment variable to point to your stack input file before running `pulumi up`. This ensures that the Pulumi modules have access to the necessary configuration for your deployment.
