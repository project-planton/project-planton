...
func init() {
	...
	Destroy.PersistentFlags().StringP(string(flag.Manifest), "f", "", "path of the deployment-component manifest file")

	Destroy.PersistentFlags().String(string(flag.InputDir), "", "directory containing target.yaml and credential yaml files")
	Destroy.PersistentFlags().String(string(flag.KustomizeDir), "", "directory containing kustomize configuration")
	Destroy.PersistentFlags().String(string(flag.Overlay), "", "kustomize overlay to use (e.g., prod, dev, staging)")
	Destroy.PersistentFlags().String(string(flag.ModuleDir), pwd, "directory containing the provisioner module")
	Destroy.PersistentFlags().StringToString(string(flag.Set), map[string]string{}, "override resource manifest values using key=value pairs")

	// Pulumi-specific flags
	Destroy.PersistentFlags().String(string(flag.Stack), "", "pulumi stack fqdn in the format of <org>/<project>/<stack>")
	Destroy.PersistentFlags().Bool(string(flag.Yes), false, "Automatically approve and perform the destroy (Pulumi)")
	Destroy.PersistentFlags().Bool(string(flag.Diff), false, "Show detailed resource diffs (Pulumi)")
	...
	Destroy.PersistentFlags().String(string(flag.AwsProviderConfig), "", "path of the aws-credential file")
	Destroy.PersistentFlags().String(string(flag.AzureProviderConfig), "", "path of the azure-credential file")
	Destroy.PersistentFlags().String(string(flag.CloudflareProviderConfig), "", "path of the cloudflare-credential file")
	Destroy.PersistentFlags().String(string(flag.ConfluentProviderConfig), "", "path of the confluent-credential file")
	Destroy.PersistentFlags().String(string(flag.GcpProviderConfig), "", "path of the gcp-credential file")
	Destroy.PersistentFlags().String(string(flag.KubernetesProviderConfig), "", "path of the yaml file containing the kubernetes cluster configuration")
	Destroy.PersistentFlags().String(string(flag.SnowflakeProviderConfig), "", "path of the snowflake-credential file")
}
...
func destroyHandler(cmd *cobra.Command, args []string) {
	...
	case provisioner.ProvisionerTypePulumi:
		destroyWithPulumi(cmd, moduleDir, targetManifestPath, valueOverrides, providerConfigOptions)
	case provisioner.ProvisionerTypeTofu:
		destroyWithTofu(cmd, moduleDir, targetManifestPath, valueOverrides, providerConfigOptions)
...
func destroyWithTofu(cmd *cobra.Command, moduleDir, targetManifestPath string, valueOverrides map[string]string,
	providerConfigOptions []stackinputproviderconfig.StackInputProviderConfigOption) {
	...
	err = tofumodule.RunCommand(moduleDir, targetManifestPath, terraform.TerraformOperationType_destroy,
		valueOverrides,
		isAutoApprove,
		false,
