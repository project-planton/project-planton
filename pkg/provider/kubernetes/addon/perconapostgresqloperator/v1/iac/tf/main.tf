...
locals {
  namespace = var.spec.namespace != "" ? var.spec.namespace : "percona-postgresql-operator"
}

resource "kubernetes_namespace" "percona_postgresql_operator" {
  metadata {
...
resource "helm_release" "percona_postgresql_operator" {
  ...
  chart      = "pg-operator"
  version    = "2.7.0"
  namespace  = kubernetes_namespace.percona_postgresql_operator.metadata[0].name
...
output "namespace" {
  ...
  value       = kubernetes_namespace.percona_postgresql_operator.metadata[0].name
}
