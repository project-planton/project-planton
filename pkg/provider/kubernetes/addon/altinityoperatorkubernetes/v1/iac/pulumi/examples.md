...
Create a file named `altinity-operator-default.yaml`:
...
```bash
planton pulumi up --manifest altinity-operator-default.yaml
```
...
Create a file named `altinity-operator-large.yaml`:
...
spec:
  ...
  namespace: altinity-operator-prod  # Custom namespace for production
  container:
    resources:
      requests:
        cpu: 200m
...
```bash
planton pulumi up --manifest altinity-operator-large.yaml
```
...
Create a file named `altinity-operator-dev.yaml`:
...
```bash
planton pulumi up --manifest altinity-operator-dev.yaml
```
...
```bash
planton pulumi up --manifest altinity-operator-updated.yaml
```
...
```bash
planton pulumi destroy --manifest altinity-operator.yaml
```
...
```bash
PULUMI_DEBUG_COMMANDS=true planton pulumi up --manifest altinity-operator.yaml
```

Check Pulumi preview before applying:

```bash
planton pulumi preview --manifest altinity-operator.yaml
```
...
```bash
cd provider/kubernetes/addon/altinityoperatorkubernetes/v1/iac/pulumi
...
1. **Verify Operator Health**:
   ```bash
   kubectl get pods -n altinity-operator
   kubectl describe pod -n altinity-operator <operator-pod-name>
   ```
...
```bash
kubectl get helmreleases -n altinity-operator
