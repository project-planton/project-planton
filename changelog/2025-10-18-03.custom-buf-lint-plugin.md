...
```
project/planton/credential/awscredential/v1/spec.proto:68:3:Field "region" has a default value but is not marked as optional. Scalar fields with (project.planton.shared.options.default) must use the 'optional' keyword to enable proper field presence detection.
...
```protobuf
// apis/project/planton/credential/awscredential/v1/spec.proto
message AwsProviderConfig {
  // VIOLATION: Has default but not optional
  string region = 4 [(planton.shared.options.default) = "us-west-2"];
}
```
...
message AwsProviderConfig {
  ...
  optional string region = 4 [(planton.shared.options.default) = "us-west-2"];
}
```
...
**File**: `apis/project/planton/credential/awscredential/v1/spec.proto`  
**Line**: 68  
...
**Before**:
```protobuf
string username = 9 [(planton.shared.options.default) = "master"];
```
...
### Proto Fixes
- `/apis/project/planton/credential/awscredential/v1/spec.proto` - Added `optional` to `region` field
