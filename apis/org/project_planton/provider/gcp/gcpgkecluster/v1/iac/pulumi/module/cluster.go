...
import (
	...
	"github.com/project-planton/project-planton/apis/org/project_planton/provider/gcp/gcpgkecluster/v1/iac/pulumi/module/vars"
...
//
// Returns:
// - *container.Cluster: A pointer to the created GCP GKE Cluster object.
...
func cluster(ctx *pulumi.Context, locals *Locals, gcpProvider *gcp.Provider) (*container.Cluster, error) {
	...
	createdFirewall, err := compute.NewFirewall(ctx, "firewall", &compute.FirewallArgs{
		Name:    pulumi.Sprintf("%s-gke-webhook", locals.GcpGkeCluster.Metadata.Name),
		Project: pulumi.String(locals.GcpGkeCluster.Spec.ClusterProjectId),
	...
	createdRouterNatIp, err := compute.NewAddress(ctx,
		"router-nat-ip",
		&compute.AddressArgs{
			Name:        pulumi.Sprintf("%s-router-nat", locals.GcpGkeCluster.Metadata.Name),
			Project:     pulumi.String(locals.GcpGkeCluster.Spec.ClusterProjectId),
	...
	//determine autoscaling input based on gcp-gke-cluster input spec
	if locals.GcpGkeCluster.Spec.ClusterAutoscalingConfig != nil &&
		locals.GcpGkeCluster.Spec.ClusterAutoscalingConfig.IsEnabled {
		clusterAutoscalingArgs = &container.ClusterClusterAutoscalingArgs{
			...
			ResourceLimits: container.ClusterClusterAutoscalingResourceLimitArray{
				container.ClusterClusterAutoscalingResourceLimitArgs{
					...
					Minimum:      pulumi.Int(locals.GcpGkeCluster.Spec.ClusterAutoscalingConfig.CpuMinCores),
					Maximum:      pulumi.Int(locals.GcpGkeCluster.Spec.ClusterAutoscalingConfig.CpuMaxCores),
				},
				container.ClusterClusterAutoscalingResourceLimitArgs{
					ResourceType: pulumi.String("memory"),
					Minimum:      pulumi.Int(locals.GcpGkeCluster.Spec.ClusterAutoscalingConfig.MemoryMinGb),
					Maximum:      pulumi.Int(locals.GcpGkeCluster.Spec.ClusterAutoscalingConfig.MemoryMaxGb),
				},
			},
		}
	}
		...
		&container.ClusterArgs{
			...
			WorkloadIdentityConfig: container.ClusterWorkloadIdentityConfigPtrInput(
				&container.ClusterWorkloadIdentityConfigArgs{
					WorkloadPool: pulumi.Sprintf("%s.svc.id.goog", locals.GcpGkeCluster.Spec.ClusterProjectId),
				}),
