...
func daemonSet(ctx *pulumi.Context, locals *Locals, kubernetesProvider pulumi.ProviderResource) error {
	...
	if target.Spec.Container.App.Env != nil {
		...
		if target.Spec.Container.App.Env.Secrets != nil {
			sortedSecretKeys := sortstringmap.SortMap(target.Spec.Container.App.Env.Secrets)
	...
	for _, vm := range target.Spec.Container.App.VolumeMounts {
		...
		if vm.HostPath != "" {
			hostPathArgs := &kubernetescorev1.HostPathVolumeSourceArgs{
				Path: pulumi.String(vm.HostPath),
			}
			if vm.HostPathType != "" {
				hostPathArgs.Type = pulumi.StringPtr(vm.HostPathType)
			}
			volumeArgs := &kubernetescorev1.VolumeArgs{
				Name:     pulumi.String(vm.Name),
				HostPath: hostPathArgs,
			}
