...
For years, the conventional wisdom was simple: managing TLS certificates in Kubernetes meant either manually uploading them as secrets or building custom automation. Then Let's Encrypt democratized certificate issuance with its free, automated ACME protocol, and cert-manager emerged as the Kubernetes-native way to bridge these worlds.

But here's what changed: **deploying cert-manager correctly** has become just as important as having it installed. A cert-manager deployment without proper DNS provider integration, high availability, or secure credential management isn't production-ready—it's a time bomb waiting for certificates to expire during an outage.

This document explains how deployment methods for cert-manager evolved from simple `kubectl apply` commands to production-grade, multi-cloud automation patterns. More importantly, it explains **why** Project Planton chose to abstract away the complexity of ClusterIssuer creation and DNS provider integration while maintaining the flexibility production environments demand.

## The Deployment Maturity Spectrum

### Level 0: Static Manifests (Quick Start, Not Production)

The simplest path is applying cert-manager's official YAML bundle directly:

```bash
kubectl apply -f https://github.com/cert-manager/cert-manager/releases/download/v1.19.1/cert-manager.yaml
```

This works. It installs cert-manager with all necessary CRDs, RBAC, and components in one command. But it's the deployment equivalent of hardcoding configuration: **what you gain in simplicity, you lose in maintainability**.
...
- Ensuring CRDs are updated *before* cert-manager components (a common failure mode)
...
Using the official Helm chart is what the cert-manager project calls a "first-class" installation method:
...
Helm brings structure: versioned releases, values-based configuration, atomic upgrades, and rollback capability. The official chart (published as OCI at `quay.io/jetstack/charts/cert-manager`) is well-maintained and handles CRD lifecycle properly when configured correctly.
...
Storing cert-manager's Helm release configuration in Git and using Argo CD or Flux to apply it brings declarative, auditable deployments:
...
- Version compatibility across the toolchain (Helm, Argo/Flux, cert-manager)
...
This is where deployment meets integration. Instead of deploying cert-manager as a standalone component and then manually wiring up ClusterIssuers, DNS credentials, and cloud provider integrations, **automation handles the entire lifecycle**.

The pattern: a higher-level API that accepts intent ("I want cert-manager with Cloudflare DNS-01 for these domains") and produces a complete, working system—deployment, credentials, ClusterIssuers, and all.
...
cert-manager's power lies in DNS-01 ACME challenges: prove you control a domain by creating a specific TXT record. But that proof requires giving cert-manager permission to create DNS records, and each cloud provider has different authentication patterns.
...
This shift from static credentials to ambient identity **eliminates secret sprawl** and reduces the blast radius of compromised credentials. The cert-manager pod authenticates using its Kubernetes ServiceAccount, which cloud metadata services map to cloud-provider IAM roles.
...
The key insight: **cert-manager supports solver multiplexing**. A single ClusterIssuer can have multiple DNS-01 solvers, each with different authentication and DNS zones. When a Certificate is requested, cert-manager selects the appropriate solver based on domain matching.
...
Project Planton chose cert-manager as the certificate automation engine—there's no viable alternative with cert-manager's maturity, community, and CNCF backing. But deploying cert-manager is table stakes. **The differentiation is integration automation.**

### The 80/20 Principle in Action

Most production cert-manager deployments configure:
- **Version**: Usually latest stable (v1.19.1 as of this writing, minimum v1.16.4 for Cloudflare compatibility)
- **Namespace**: `cert-manager` (convention)
...
For **Cloudflare**: API tokens are stored as Kubernetes Secrets (no avoiding this). Project Planton creates the secret in the correct namespace with the correct key name that cert-manager expects.

For **cloud providers** (GCP/AWS/Azure): Project Planton configures the cert-manager ServiceAccount with the appropriate workload identity annotations. The cloud provider's metadata service handles authentication. No static credentials stored in Kubernetes.
...
Production cert-manager requires:
...
A common cert-manager failure mode: **DNS propagation timeouts**. cert-manager creates a TXT record via the DNS provider API, but then can't verify it exists because it's querying the wrong nameservers (often in-cluster CoreDNS with stale caches).

The fix: Configure cert-manager to use **public recursive nameservers** (Cloudflare 1.1.1.1 and Google 8.8.8.8) for DNS propagation checks. This is a flag in the cert-manager deployment:
...
Not all cert-manager versions are created equal. **Version v1.16.4** fixed critical Cloudflare API compatibility issues where DNS records weren't properly cleaned up after challenges, causing subsequent challenges to fail.
...
Even with automation, understanding how cert-manager works in production prevents surprises.
...
cert-manager automatically renews certificates **30 days before expiration** (configurable via `renewBefore`). The renewal process:
...
**Critical insight**: Applications must reload certificates when the Secret changes. Ingress controllers (nginx-ingress, Traefik) handle this automatically. Custom applications need to watch the Secret and reload, or use cert-manager's CSI driver for automatic file updates.

**Monitoring**: Track certificate expiration using Prometheus metrics (`certmanager_certificate_expiration_timestamp_seconds`) or external monitoring. While auto-renewal should work, monitoring ensures you catch failures before outages.
...
- Avoid reissuing certificates unnecessarily (cert-manager's auto-renewal handles this)
...
**"Waiting for DNS propagation"**: The TXT record exists but cert-manager can't verify it. Usually resolves in 1-3 minutes. If stuck longer, check:
...
- cert-manager logs for DNS resolver issues
...
cert-manager is **Apache 2.0 licensed**, free to use, modify, and distribute. It's a CNCF incubating project (since 2020), governed by The Linux Foundation.

**Official container images**: Published at `quay.io/jetstack/cert-manager-*` (controller, webhook, cainjector). Also available as OCI artifacts.
...
CyberArk provides **LTS cert-manager on AWS Marketplace** as an EKS add-on, offering:
- Subscription-based support
- FIPS-compliant builds
- Integration with AWS billing

**Alternative solutions**: Before cert-manager, projects like **kube-lego** and **kube-cert-manager** existed but are now deprecated. Today, cert-manager has no serious competition for Kubernetes certificate automation. External tools like Certbot or cloud-native certificate managers (AWS ACM, GCP Certificate Manager) serve different use cases (external automation or cloud load balancer integration, respectively).
...
**The result**: You declare intent ("I want cert-manager with these DNS providers"). Project Planton produces a complete, working system. When you need to add a domain or rotate credentials, you update the spec and redeploy. The integration layer handles the rest.
...
Deploying cert-manager is straightforward. Integrating it correctly—with secure DNS provider authentication, high availability, multi-cloud support, and operational best practices—is not. That integration complexity is undifferentiated heavy lifting: every team solves the same problems, writes the same ClusterIssuer YAML, makes the same mistakes with DNS propagation and credential management.
...
That's the paradigm shift from deployment to automation: moving from "how do I install cert-manager?" to "how do I enable certificate automation for my domains across my infrastructure?". Project Planton answers the latter by handling the former as an implementation detail.
