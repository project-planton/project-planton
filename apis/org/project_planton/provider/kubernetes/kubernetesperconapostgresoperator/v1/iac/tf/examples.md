# KubernetesPerconaPostgresOperator Terraform Module Examples
...
terraform {
  backend "s3" {
    ...
    key            = "kubernetes-percona-postgres-operator/terraform.tfstate"
    region         = "us-west-2"
    encrypt        = true
...
```bash
# Check namespace
kubectl get namespace kubernetes-percona-postgres-operator
...
```hcl
terraform {
  source = "git::https://github.com/myorg/terraform-modules//kubernetes-percona-postgres-operator"
}
