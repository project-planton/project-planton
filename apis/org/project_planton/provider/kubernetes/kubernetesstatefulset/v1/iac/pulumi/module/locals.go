...
type Locals struct {
	...
	Namespace                    string
	KubernetesStatefulSet        *kubernetesstatefulsetv1.KubernetesStatefulSet
...
func initializeLocals(ctx *pulumi.Context, stackInput *kubernetesstatefulsetv1.KubernetesStatefulSetStackInput) (*Locals, error) {
	locals := &Locals{}
	...
	// Headless service name (for stable network identity)
	locals.HeadlessServiceName = fmt.Sprintf("%s-headless", target.Metadata.Name)

	// Export headless service name
	ctx.Export(OpHeadlessService, pulumi.String(locals.HeadlessServiceName))
	...
	if locals.KubernetesStatefulSet.Spec.Container.App.Image == nil {
		return nil, errors.New("spec.container.app.image is required")
	}

	if locals.KubernetesStatefulSet.Spec.Availability == nil {
		locals.KubernetesStatefulSet.Spec.Availability = &kubernetesstatefulsetv1.KubernetesStatefulSetAvailability{
			Replicas: 1,
		}
	}
