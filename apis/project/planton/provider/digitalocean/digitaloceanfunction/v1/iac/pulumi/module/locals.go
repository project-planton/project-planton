...
func initializeLocals(_ *pulumi.Context, stackInput *digitaloceanfunctionv1.DigitalOceanFunctionStackInput) *Locals {
	...
	if locals.DigitalOceanFunction.Metadata.Org != "" {
		locals.DoLabels[digitaloceanlabelkeys.Organization] = locals.DigitalOceanFunction.Metadata.Org
	}

	if locals.DigitalOceanFunction.Metadata.Env != "" {
		locals.DoLabels[digitaloceanlabelkeys.Environment] = locals.DigitalOceanFunction.Metadata.Env
	}

	if locals.DigitalOceanFunction.Metadata.Id != "" {
		locals.DoLabels[digitaloceanlabelkeys.ResourceId] = locals.DigitalOceanFunction.Metadata.Id
	}
