// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.34.2
// 	protoc        (unknown)
// source: project/planton/provider/aws/ecscluster/v1/spec.proto

package ecsclusterv1

import (
	_ "buf.build/gen/go/bufbuild/protovalidate/protocolbuffers/go/buf/validate"
	_ "github.com/project-planton/project-planton/apis/project/planton/shared/options"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// EcsClusterLaunchType enumerates the possible ways to run ECS tasks.
type EcsClusterLaunchType int32

const (
	// ECS_LAUNCH_TYPE_UNSPECIFIED means the field was not specified; the system treats it as FARGATE by default.
	EcsClusterLaunchType_ecs_launch_type_unspecified EcsClusterLaunchType = 0
	// FARGATE runs tasks using AWS Fargate (serverless).
	EcsClusterLaunchType_fargate EcsClusterLaunchType = 1
	// EC2 runs tasks on user-managed Amazon EC2 instances.
	EcsClusterLaunchType_ec2 EcsClusterLaunchType = 2
)

// Enum value maps for EcsClusterLaunchType.
var (
	EcsClusterLaunchType_name = map[int32]string{
		0: "ecs_launch_type_unspecified",
		1: "fargate",
		2: "ec2",
	}
	EcsClusterLaunchType_value = map[string]int32{
		"ecs_launch_type_unspecified": 0,
		"fargate":                     1,
		"ec2":                         2,
	}
)

func (x EcsClusterLaunchType) Enum() *EcsClusterLaunchType {
	p := new(EcsClusterLaunchType)
	*p = x
	return p
}

func (x EcsClusterLaunchType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (EcsClusterLaunchType) Descriptor() protoreflect.EnumDescriptor {
	return file_project_planton_provider_aws_ecscluster_v1_spec_proto_enumTypes[0].Descriptor()
}

func (EcsClusterLaunchType) Type() protoreflect.EnumType {
	return &file_project_planton_provider_aws_ecscluster_v1_spec_proto_enumTypes[0]
}

func (x EcsClusterLaunchType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use EcsClusterLaunchType.Descriptor instead.
func (EcsClusterLaunchType) EnumDescriptor() ([]byte, []int) {
	return file_project_planton_provider_aws_ecscluster_v1_spec_proto_rawDescGZIP(), []int{0}
}

// EcsClusterSpec defines the configuration for deploying a containerized application on AWS ECS.
// This covers both Fargate (serverless) and EC2 (self-managed) launch types by exposing a 'launch_type' field.
type EcsClusterSpec struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// launch_type specifies whether to run on FARGATE or EC2.
	// By default, if not provided, we treat it as FARGATE (serverless).
	// Valid values are ["FARGATE", "EC2"].
	LaunchType EcsClusterLaunchType `protobuf:"varint,1,opt,name=launch_type,json=launchType,proto3,enum=project.planton.provider.aws.ecscluster.v1.EcsClusterLaunchType" json:"launch_type,omitempty"`
	// cluster_name indicates the name or ARN of the ECS cluster to deploy into.
	// If empty, the platform may choose a default cluster (e.g., derived from environment).
	ClusterName string `protobuf:"bytes,2,opt,name=cluster_name,json=clusterName,proto3" json:"cluster_name,omitempty"`
	// container_image is the Docker image for your service, e.g.,
	// "123456789012.dkr.ecr.us-east-1.amazonaws.com/myapp:latest" or "nginx:latest".
	// This is required, as ECS must know what container image to run.
	ContainerImage string `protobuf:"bytes,3,opt,name=container_image,json=containerImage,proto3" json:"container_image,omitempty"`
	// container_port is the port on which the application listens inside the container.
	// Co-pilot will show "8080" as a recommended placeholder, but the user can override.
	ContainerPort int32 `protobuf:"varint,4,opt,name=container_port,json=containerPort,proto3" json:"container_port,omitempty"`
	// cpu is the number of CPU units for the ECS task.
	// For Fargate, allowed values include 256, 512, 1024, etc.
	// Co-pilot will suggest "256" as a starting value.
	Cpu int32 `protobuf:"varint,5,opt,name=cpu,proto3" json:"cpu,omitempty"`
	// memory is the amount of memory (in MiB) for the ECS task.
	// For Fargate, 512, 1024, and similar increments are valid combos.
	// Co-pilot will suggest "512" by default, but the user can override.
	Memory int32 `protobuf:"varint,6,opt,name=memory,proto3" json:"memory,omitempty"`
	// desired_count is the desired number of running tasks for this service.
	// Co-pilot will propose "1" by default.
	DesiredCount int32 `protobuf:"varint,7,opt,name=desired_count,json=desiredCount,proto3" json:"desired_count,omitempty"`
	// environment_variables is a map of key/value pairs for non-sensitive config,
	// injected as container environment variables. Example: {"LOG_LEVEL": "debug"}.
	EnvironmentVariables map[string]string `protobuf:"bytes,8,rep,name=environment_variables,json=environmentVariables,proto3" json:"environment_variables,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"bytes,2,opt,name=value,proto3"`
	// secret_variables is a map of key/value references to AWS Secrets Manager or SSM
	// for sensitive data, e.g. {"DB_PASSWORD": "arn:aws:ssm:us-east-1:..."}.
	// The platform resolves these references into secure environment variables at runtime.
	SecretVariables map[string]string `protobuf:"bytes,9,rep,name=secret_variables,json=secretVariables,proto3" json:"secret_variables,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"bytes,2,opt,name=value,proto3"`
	// network defines VPC, subnet, and security group configuration for this ECS service.
	// If empty, the platform may provide sensible defaults (e.g., a shared VPC).
	Network *EcsClusterNetwork `protobuf:"bytes,10,opt,name=network,proto3" json:"network,omitempty"`
	// ingress controls how the service is exposed externally (via ALB, domain, etc.)
	// or whether it remains private (is_public=false).
	Ingress *EcsClusterIngress `protobuf:"bytes,11,opt,name=ingress,proto3" json:"ingress,omitempty"`
	// auto_scaling optionally configures horizontal scaling (min/max tasks).
	// If is_enabled=false, the service runs at the desired_count only.
	AutoScaling *EcsClusterAutoScaling `protobuf:"bytes,12,opt,name=auto_scaling,json=autoScaling,proto3" json:"auto_scaling,omitempty"`
}

func (x *EcsClusterSpec) Reset() {
	*x = EcsClusterSpec{}
	if protoimpl.UnsafeEnabled {
		mi := &file_project_planton_provider_aws_ecscluster_v1_spec_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *EcsClusterSpec) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EcsClusterSpec) ProtoMessage() {}

func (x *EcsClusterSpec) ProtoReflect() protoreflect.Message {
	mi := &file_project_planton_provider_aws_ecscluster_v1_spec_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EcsClusterSpec.ProtoReflect.Descriptor instead.
func (*EcsClusterSpec) Descriptor() ([]byte, []int) {
	return file_project_planton_provider_aws_ecscluster_v1_spec_proto_rawDescGZIP(), []int{0}
}

func (x *EcsClusterSpec) GetLaunchType() EcsClusterLaunchType {
	if x != nil {
		return x.LaunchType
	}
	return EcsClusterLaunchType_ecs_launch_type_unspecified
}

func (x *EcsClusterSpec) GetClusterName() string {
	if x != nil {
		return x.ClusterName
	}
	return ""
}

func (x *EcsClusterSpec) GetContainerImage() string {
	if x != nil {
		return x.ContainerImage
	}
	return ""
}

func (x *EcsClusterSpec) GetContainerPort() int32 {
	if x != nil {
		return x.ContainerPort
	}
	return 0
}

func (x *EcsClusterSpec) GetCpu() int32 {
	if x != nil {
		return x.Cpu
	}
	return 0
}

func (x *EcsClusterSpec) GetMemory() int32 {
	if x != nil {
		return x.Memory
	}
	return 0
}

func (x *EcsClusterSpec) GetDesiredCount() int32 {
	if x != nil {
		return x.DesiredCount
	}
	return 0
}

func (x *EcsClusterSpec) GetEnvironmentVariables() map[string]string {
	if x != nil {
		return x.EnvironmentVariables
	}
	return nil
}

func (x *EcsClusterSpec) GetSecretVariables() map[string]string {
	if x != nil {
		return x.SecretVariables
	}
	return nil
}

func (x *EcsClusterSpec) GetNetwork() *EcsClusterNetwork {
	if x != nil {
		return x.Network
	}
	return nil
}

func (x *EcsClusterSpec) GetIngress() *EcsClusterIngress {
	if x != nil {
		return x.Ingress
	}
	return nil
}

func (x *EcsClusterSpec) GetAutoScaling() *EcsClusterAutoScaling {
	if x != nil {
		return x.AutoScaling
	}
	return nil
}

// EcsClusterNetwork defines VPC, subnet, and security group settings for the ECS tasks.
type EcsClusterNetwork struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// vpc_id is the ID of the VPC where tasks will run (e.g. "vpc-0123456789abcdef0").
	// If left empty, the platform may pick a default VPC.
	VpcId string `protobuf:"bytes,1,opt,name=vpc_id,json=vpcId,proto3" json:"vpc_id,omitempty"`
	// subnet_ids is a list of subnet IDs where tasks will run.
	// Typically private subnets for production or public if you want direct internet traffic.
	SubnetIds []string `protobuf:"bytes,2,rep,name=subnet_ids,json=subnetIds,proto3" json:"subnet_ids,omitempty"`
	// security_group_ids is a list of security groups attached to each task's ENI.
	// If omitted, a default security group might be assigned.
	SecurityGroupIds []string `protobuf:"bytes,3,rep,name=security_group_ids,json=securityGroupIds,proto3" json:"security_group_ids,omitempty"`
	// assign_public_ip indicates whether to allocate a public IP for tasks (Fargate in a public subnet).
	AssignPublicIp bool `protobuf:"varint,4,opt,name=assign_public_ip,json=assignPublicIp,proto3" json:"assign_public_ip,omitempty"`
}

func (x *EcsClusterNetwork) Reset() {
	*x = EcsClusterNetwork{}
	if protoimpl.UnsafeEnabled {
		mi := &file_project_planton_provider_aws_ecscluster_v1_spec_proto_msgTypes[1]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *EcsClusterNetwork) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EcsClusterNetwork) ProtoMessage() {}

func (x *EcsClusterNetwork) ProtoReflect() protoreflect.Message {
	mi := &file_project_planton_provider_aws_ecscluster_v1_spec_proto_msgTypes[1]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EcsClusterNetwork.ProtoReflect.Descriptor instead.
func (*EcsClusterNetwork) Descriptor() ([]byte, []int) {
	return file_project_planton_provider_aws_ecscluster_v1_spec_proto_rawDescGZIP(), []int{1}
}

func (x *EcsClusterNetwork) GetVpcId() string {
	if x != nil {
		return x.VpcId
	}
	return ""
}

func (x *EcsClusterNetwork) GetSubnetIds() []string {
	if x != nil {
		return x.SubnetIds
	}
	return nil
}

func (x *EcsClusterNetwork) GetSecurityGroupIds() []string {
	if x != nil {
		return x.SecurityGroupIds
	}
	return nil
}

func (x *EcsClusterNetwork) GetAssignPublicIp() bool {
	if x != nil {
		return x.AssignPublicIp
	}
	return false
}

// EcsClusterIngress defines how a service is exposed externally via AWS load balancers or domains.
type EcsClusterIngress struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// is_public indicates if the service is internet-facing. If false, the service might be internal-only.
	IsPublic bool `protobuf:"varint,1,opt,name=is_public,json=isPublic,proto3" json:"is_public,omitempty"`
	// domain_name is an optional fully qualified domain, e.g. "api.example.com".
	// If omitted, the platform may provide a default LB DNS or keep it internal.
	DomainName string `protobuf:"bytes,2,opt,name=domain_name,json=domainName,proto3" json:"domain_name,omitempty"`
	// path is the optional path for path-based routing (e.g., "/api/*").
	// If omitted, default is "/" routing for the entire domain.
	Path string `protobuf:"bytes,3,opt,name=path,proto3" json:"path,omitempty"`
	// health_check_path is where the ALB health check probes. If empty, defaults to "/".
	HealthCheckPath string `protobuf:"bytes,4,opt,name=health_check_path,json=healthCheckPath,proto3" json:"health_check_path,omitempty"`
}

func (x *EcsClusterIngress) Reset() {
	*x = EcsClusterIngress{}
	if protoimpl.UnsafeEnabled {
		mi := &file_project_planton_provider_aws_ecscluster_v1_spec_proto_msgTypes[2]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *EcsClusterIngress) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EcsClusterIngress) ProtoMessage() {}

func (x *EcsClusterIngress) ProtoReflect() protoreflect.Message {
	mi := &file_project_planton_provider_aws_ecscluster_v1_spec_proto_msgTypes[2]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EcsClusterIngress.ProtoReflect.Descriptor instead.
func (*EcsClusterIngress) Descriptor() ([]byte, []int) {
	return file_project_planton_provider_aws_ecscluster_v1_spec_proto_rawDescGZIP(), []int{2}
}

func (x *EcsClusterIngress) GetIsPublic() bool {
	if x != nil {
		return x.IsPublic
	}
	return false
}

func (x *EcsClusterIngress) GetDomainName() string {
	if x != nil {
		return x.DomainName
	}
	return ""
}

func (x *EcsClusterIngress) GetPath() string {
	if x != nil {
		return x.Path
	}
	return ""
}

func (x *EcsClusterIngress) GetHealthCheckPath() string {
	if x != nil {
		return x.HealthCheckPath
	}
	return ""
}

// EcsClusterAutoScaling configures optional horizontal scaling based on CPU usage, memory usage, or other metrics.
type EcsClusterAutoScaling struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// is_enabled determines if auto-scaling is active.
	// When false, the service remains at "desired_count" tasks.
	IsEnabled bool `protobuf:"varint,1,opt,name=is_enabled,json=isEnabled,proto3" json:"is_enabled,omitempty"`
	// min_count is the minimum number of tasks to run under auto-scaling.
	// If 0 is valid, you can run zero tasks, but that means no requests are served.
	MinCount int32 `protobuf:"varint,2,opt,name=min_count,json=minCount,proto3" json:"min_count,omitempty"`
	// max_count is the maximum number of tasks.
	// If you set it too high, you could scale out significantly based on usage.
	MaxCount int32 `protobuf:"varint,3,opt,name=max_count,json=maxCount,proto3" json:"max_count,omitempty"`
}

func (x *EcsClusterAutoScaling) Reset() {
	*x = EcsClusterAutoScaling{}
	if protoimpl.UnsafeEnabled {
		mi := &file_project_planton_provider_aws_ecscluster_v1_spec_proto_msgTypes[3]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *EcsClusterAutoScaling) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EcsClusterAutoScaling) ProtoMessage() {}

func (x *EcsClusterAutoScaling) ProtoReflect() protoreflect.Message {
	mi := &file_project_planton_provider_aws_ecscluster_v1_spec_proto_msgTypes[3]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EcsClusterAutoScaling.ProtoReflect.Descriptor instead.
func (*EcsClusterAutoScaling) Descriptor() ([]byte, []int) {
	return file_project_planton_provider_aws_ecscluster_v1_spec_proto_rawDescGZIP(), []int{3}
}

func (x *EcsClusterAutoScaling) GetIsEnabled() bool {
	if x != nil {
		return x.IsEnabled
	}
	return false
}

func (x *EcsClusterAutoScaling) GetMinCount() int32 {
	if x != nil {
		return x.MinCount
	}
	return 0
}

func (x *EcsClusterAutoScaling) GetMaxCount() int32 {
	if x != nil {
		return x.MaxCount
	}
	return 0
}

var File_project_planton_provider_aws_ecscluster_v1_spec_proto protoreflect.FileDescriptor

var file_project_planton_provider_aws_ecscluster_v1_spec_proto_rawDesc = []byte{
	,
}

var (
	file_project_planton_provider_aws_ecscluster_v1_spec_proto_rawDescOnce sync.Once
	file_project_planton_provider_aws_ecscluster_v1_spec_proto_rawDescData = file_project_planton_provider_aws_ecscluster_v1_spec_proto_rawDesc
)

func file_project_planton_provider_aws_ecscluster_v1_spec_proto_rawDescGZIP() []byte {
	file_project_planton_provider_aws_ecscluster_v1_spec_proto_rawDescOnce.Do(func() {
		file_project_planton_provider_aws_ecscluster_v1_spec_proto_rawDescData = protoimpl.X.CompressGZIP(file_project_planton_provider_aws_ecscluster_v1_spec_proto_rawDescData)
	})
	return file_project_planton_provider_aws_ecscluster_v1_spec_proto_rawDescData
}

var file_project_planton_provider_aws_ecscluster_v1_spec_proto_enumTypes = make([]protoimpl.EnumInfo, 1)
var file_project_planton_provider_aws_ecscluster_v1_spec_proto_msgTypes = make([]protoimpl.MessageInfo, 6)
var file_project_planton_provider_aws_ecscluster_v1_spec_proto_goTypes = []any{
	(EcsClusterLaunchType)(0),     // 0: project.planton.provider.aws.ecscluster.v1.EcsClusterLaunchType
	(*EcsClusterSpec)(nil),        // 1: project.planton.provider.aws.ecscluster.v1.EcsClusterSpec
	(*EcsClusterNetwork)(nil),     // 2: project.planton.provider.aws.ecscluster.v1.EcsClusterNetwork
	(*EcsClusterIngress)(nil),     // 3: project.planton.provider.aws.ecscluster.v1.EcsClusterIngress
	(*EcsClusterAutoScaling)(nil), // 4: project.planton.provider.aws.ecscluster.v1.EcsClusterAutoScaling
	nil,                           // 5: project.planton.provider.aws.ecscluster.v1.EcsClusterSpec.EnvironmentVariablesEntry
	nil,                           // 6: project.planton.provider.aws.ecscluster.v1.EcsClusterSpec.SecretVariablesEntry
}
var file_project_planton_provider_aws_ecscluster_v1_spec_proto_depIdxs = []int32{
	0, // 0: project.planton.provider.aws.ecscluster.v1.EcsClusterSpec.launch_type:type_name -> project.planton.provider.aws.ecscluster.v1.EcsClusterLaunchType
	5, // 1: project.planton.provider.aws.ecscluster.v1.EcsClusterSpec.environment_variables:type_name -> project.planton.provider.aws.ecscluster.v1.EcsClusterSpec.EnvironmentVariablesEntry
	6, // 2: project.planton.provider.aws.ecscluster.v1.EcsClusterSpec.secret_variables:type_name -> project.planton.provider.aws.ecscluster.v1.EcsClusterSpec.SecretVariablesEntry
	2, // 3: project.planton.provider.aws.ecscluster.v1.EcsClusterSpec.network:type_name -> project.planton.provider.aws.ecscluster.v1.EcsClusterNetwork
	3, // 4: project.planton.provider.aws.ecscluster.v1.EcsClusterSpec.ingress:type_name -> project.planton.provider.aws.ecscluster.v1.EcsClusterIngress
	4, // 5: project.planton.provider.aws.ecscluster.v1.EcsClusterSpec.auto_scaling:type_name -> project.planton.provider.aws.ecscluster.v1.EcsClusterAutoScaling
	6, // [6:6] is the sub-list for method output_type
	6, // [6:6] is the sub-list for method input_type
	6, // [6:6] is the sub-list for extension type_name
	6, // [6:6] is the sub-list for extension extendee
	0, // [0:6] is the sub-list for field type_name
}

func init() { file_project_planton_provider_aws_ecscluster_v1_spec_proto_init() }
func file_project_planton_provider_aws_ecscluster_v1_spec_proto_init() {
	if File_project_planton_provider_aws_ecscluster_v1_spec_proto != nil {
		return
	}
	if !protoimpl.UnsafeEnabled {
		file_project_planton_provider_aws_ecscluster_v1_spec_proto_msgTypes[0].Exporter = func(v any, i int) any {
			switch v := v.(*EcsClusterSpec); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_project_planton_provider_aws_ecscluster_v1_spec_proto_msgTypes[1].Exporter = func(v any, i int) any {
			switch v := v.(*EcsClusterNetwork); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_project_planton_provider_aws_ecscluster_v1_spec_proto_msgTypes[2].Exporter = func(v any, i int) any {
			switch v := v.(*EcsClusterIngress); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_project_planton_provider_aws_ecscluster_v1_spec_proto_msgTypes[3].Exporter = func(v any, i int) any {
			switch v := v.(*EcsClusterAutoScaling); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_project_planton_provider_aws_ecscluster_v1_spec_proto_rawDesc,
			NumEnums:      1,
			NumMessages:   6,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_project_planton_provider_aws_ecscluster_v1_spec_proto_goTypes,
		DependencyIndexes: file_project_planton_provider_aws_ecscluster_v1_spec_proto_depIdxs,
		EnumInfos:         file_project_planton_provider_aws_ecscluster_v1_spec_proto_enumTypes,
		MessageInfos:      file_project_planton_provider_aws_ecscluster_v1_spec_proto_msgTypes,
	}.Build()
	File_project_planton_provider_aws_ecscluster_v1_spec_proto = out.File
	file_project_planton_provider_aws_ecscluster_v1_spec_proto_rawDesc = nil
	file_project_planton_provider_aws_ecscluster_v1_spec_proto_goTypes = nil
	file_project_planton_provider_aws_ecscluster_v1_spec_proto_depIdxs = nil
}
