locals {
  ...
  final_labels = merge(local.base_labels, local.org_label, env_label)

  # Merge final_labels with any user-supplied labels in var.metadata.labels (if present)
  ...
  # Example of correct multiline ternary usage
  ...
  # IAM Role config
  create_iam_role                             = var.spec.iam_role != null
