...
resource "aws_iam_role_policy" "inline" {
  ...
  policy = local.inline_policy_json
}
...
resource "aws_lambda_function" "this" {
  ...
  s3_bucket        = local.is_image ? null : local.s3_bucket
  s3_key           = local.is_image ? null : local.s3_key
  ...
  tracing_config {
    mode = try(var.spec.function.tracing_config_mode, null)
  }
  ...
  ephemeral_storage {
    size = try(var.spec.function.ephemeral_storage_size, null)
  }
